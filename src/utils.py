...
def save_simple_metrics_reports(train_score: float, test_score: float, validation_score: float, model:Pipeline)-> None:
...
def get_model_performance_test_set(y_test:pd.Series, y_test_pred:pd.Series) -> None:
    fig, ax = plt.subplots()
    fig.set_figheight(8)
    fig.set_figwidth(8)
